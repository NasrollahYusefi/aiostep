...
- **Support for Telebot and Telethon**:
  - `wait_for` and `register_next_step` now work seamlessly with *Telebot* and *Telethon*, expanding integration options with different Telegram libraries.
- New `IsState` filter in `utils`:
  - Introduced a filter for easier handling of aiogram updates, making state management more straightforward.

### Changed

- **Aiogram usage update**:
  - The `Listen` method is now accessed via `aiogram_dialect.Listen()` for improved clarity and consistency.
